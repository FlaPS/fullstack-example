...
exports.reducerFactory = function (reducer) { return function (selector) { return function (state, action) { return selector(action) ? reducer(state, action) : state; }; }; };
exports.isType = function (actionCreator) { return function (action) { return action.type === actionCreator['type']; }; };
exports.isTypeOfAny = function (actionCreator) { return function (action) { return actionCreator.some(function (creator) { return creator.type === action.type; }); }; };
exports.isHasCreatorFactory = function (acf) { return acf && acf['factory']; };
exports.isNamespace = function (actionFactory) { return function (action) { return exports.isHasCreatorFactory(actionFactory)
    ...
    function asyncActionCreators(type, commonMeta) {
        ...
        return {
            ...
            asyncReducer: reducerWithInitialState({})
                .case(started, function (state, payload) { return ({ value: undefined, error: undefined, status: 'started', params: payload }); })
                .case(done, function (state, payload) { return ({ value: payload.result, error: undefined, status: 'done', params: payload.params }); })
                .case(failed, function (state, payload) { return ({ value: undefined, error: payload.error, status: 'failed', params: payload.params }); }),
        };
    }
    ...
    var reducer = (function (state, action) {
        ...
        for (var i = 0, length_1 = cases.length; i < length_1; i++) {
            ...
            if (exports.isType(actionCreator)(action))
                return Object.assign({}, { state: state }, Object.assign({}, handler(state, action.payload)));
        }
        return state;
    });
    // @ts-ignore
